...
const static int MAX_TEST = 4999;
...
void Adjust_Number_Of_Client()
{
	...
	SOCKADDR_IN ServerAddirection;
	ZeroMemory(&ServerAddirection, sizeof(SOCKADDR_IN));
	ServerAddirection.sin_family = AF_INET;
	ServerAddirection.sin_port = htons(SERVER_PORT);
	ServerAddirection.sin_addr.s_addr = inet_addr("127.0.0.1");


	int Result = WSAConnect(g_clients[num_connections].client_socket, (sockaddr*)&ServerAddirection, sizeof(ServerAddirection), NULL, NULL, NULL, NULL);
	if (0 != Result) {
	...
	char temp[20]{};
	_itoa_s(num_connections, temp, 10);
	//sprintf_s(l_packet.name, "%d", temp);
	l_packet.size = sizeof(l_packet);
	l_packet.type = CS_LOGIN;
	strcpy_s(l_packet.player_id, "test");
	strcat_s(l_packet.player_id, temp);
...
void Test_Thread()
{
	while (true) {
		...
		for (int i = 0; i < num_connections; ++i) {
			...
			switch (rand() % 4) {
			case 0: my_packet.direction = RIGHT; break;
			case 1: my_packet.direction = LEFT; break;
			case 2: my_packet.direction = UP; break;
			case 3: my_packet.direction = DOWN; break;
			}
